...
const createItem = (ext: Extension, isUnofficial = false) => {
  let item: HTMLAnchorElement | HTMLButtonElement;

  if (ext.isInstalled || isUnofficial) {
...
create = (update?: boolean) => {
  ...
  if (unofficial.childElementCount) {
    const title = document.createElement("h2");
    title.textContent = "Unofficial";
    div.append(title, unofficial);
  }
